...
int getWakeHour() {
    int wake_hour;
    printf("Enter your wake hour (1-%d): ", DAY_HOURS - 2);
    while (scanf("%d", &wake_hour) || wake_hour < 1 || wake_hour > DAY_HOURS - 2)
        printf("Please insert valid input (1-%d): ", DAY_HOURS - 2);
...
int getSleepHour(int wakeHour) {
    ...
    printf("Enter your sleep hour (%d-%d): ", wakeHour + 1, DAY_HOURS - 1);
    while (scanf("%d", &sleep_hour) || sleep_hour <= wakeHour || sleep_hour > DAY_HOURS - 1)
        printf("Please insert valid input (%d-%d): ", wakeHour + 1, DAY_HOURS - 1);
...
int getFastingGap() {
    ...
    while (scanf("%d", &fasting_gap) || fasting_gap <= 0)
...
int getMeatToDairyWait() {
    ...
    while (scanf("%d", &meat_to_dairy_wait) || meat_to_dairy_wait < 0)
